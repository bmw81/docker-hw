...
`Образ уменьшился примерно с ~800 MB (образ golang:latest содержит компилятор, линковщик, исходники стандартной библиотеки, git) до ~15-20 MB (финальный образ alpine:latest содержит только мой скомпилированный бинарный файл app и сертификаты). В первом случае в контейнере оставались инструменты разработчика и кэш пакетов, во втором — только рантайм.`
...
**Почему ENV в Dockerfile — это утечка?**

Инструкция ENV записывается в метаданные образа. Метаданные — это часть слоя. Даже если удалить контейнер, слой с ENV MY_SECRET_TOKEN=SuperSecret123 останется в локальном кэше Docker (docker history покажет его). Злоумышленник с доступом к серверу сборки может выполнить docker image inspect или docker history и увидеть пароль в открытом виде.

**Как работает --mount=type=secret?**

Это фича BuildKit. Когда ты указываешь --secret, BuildKit НЕ ДОБАВЛЯЕТ этот файл в слой образа. Вместо этого он создает временное монтирование (tmpfs) только на время выполнения инструкции RUN. Представь, что к твоему RUN на 1 секунду подключили флешку с паролем, а после завершения команды флешку выдернули. В финальном образе файла /run/secrets/... не существует, и в docker history он не фигурирует.`
